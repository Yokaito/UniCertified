...
router.use((req, res, next) => {
  /* Middlawares que verifica se a sessao esta ativa */
...
router.get("/ativar", async (req, res) => {
  if (!req.query.token && !req.cookies["token"])
    res.render("ativacao", {
      error:
        "Token Expirado ou inexistente. Realize o Login novamente para reenviar um token para o seu email.",
      style: "controllers_css/registro.css",
    });
  else {
            ...
            User.update(
              ...
              if (update_response) {
                ...
                let mailOptions = {
                  ...
                  subject: `UniCertified | ${find_response.get(
                    "name_user"
                  )} Sua conta foi ativada com sucesso.`, // Subject line
                  template: "ativado",
                  context: {
...
router.post("/registrar", async (req, res) => {
  ...
  if (
    !(
      data.nome_usuario != " " &&
      data.nome_usuario.length >= 5 &&
      data.nome_usuario.length <= 40
    )
  )
    count_errors += 1;
  else if (
    ...
    !(
      data.senha_usuario != " " &&
      data.senha_usuario.length >= 5 &&
      data.senha_usuario.length <= 40
    )
  )
    count_errors += 1;
  else if (
    !(
      data.senha_conf_usuario != " " &&
      data.senha_conf_usuario.length >= 5 &&
      data.senha_conf_usuario.length <= 40 &&
      data.senha_conf_usuario == data.senha_usuario
    )
  )
    count_errors += 1;
...
router.get("/recuperar_senha", (req, res) => {
  if (req.query.tokenr && req.query.email) {
    if (
      req.query.tokenr == req.cookies["tokenr"] &&
      req.session.tokenr == req.query.tokenr &&
      req.session.tokenr == req.cookies["tokenr"]
    ) {
      let count_errors = 0;
      if (
        !(
...
router.post("/recuperar_senha/trocar", (req, res) => {
  ...
  if (
    req.session.tokenr == req.cookies["tokenr"] &&
    data.email_usuario == req.session.email
  ) {
    if (count_errors == 0)
      User.findOne({
        where: {
        ...
        if (response_findOne) {
          ...
          res.clearCookie("tokenr").send({
            ...
            text: "Recomeçe o processo",
            link: process.env.URL_FRONT_RECUPERAR_SENHA,
          });
        }
      });
    else {
      res.session = null;
      res.send({
        error: "Ocorreu um erro nos dados enviados",
        text: "Recomeçe o processo",
        link: process.env.URL_FRONT_RECUPERAR_SENHA,
      });
    }
    ...
    res.session = null;
    res.send({
      error: "Token ou email alterado do original",
      text: "Recomeçe o processo",
      link: process.env.URL_FRONT_RECUPERAR_SENHA,
    });
  }
    ...
    User.findOne({
      ...
      if (response_findOne) {
        let mailOptions = {
          ...
          context: {
            link: `${
              process.env.NM_CONTEXT_LINK_RECUPERAR
            }tokenr=${tokenr}&email=${response_findOne.get("email_user")}`,
        ...
        res.cookie("tokenr", tokenr, { maxAge: 600000 }).send({
