...
router.post("/auth", async (req, res) => {
  ...
  if (
    !(
      user.email_usuario != " " &&
      user.email_usuario.length >= 5 &&
      user.email_usuario.length <= 40 &&
      user.email_usuario.match(
  ...
  if (
    !(
      user.senha_usuario != " " &&
      user.senha_usuario.length >= 5 &&
      user.senha_usuario.length <= 40
    )
  )
    count_errors += 1;
      ...
      if (response_findOne) {
        if (response_findOne.get("id_activation_state_foreign") == 1) {
          if (
            bcrypt.compareSync(
              user.senha_usuario,
              response_findOne.get("password_user")
            )
          ) {
            User.update(
          ...
          User.update(
            ...
            if (update_response) {
              let mailOptions = {
                ...
                subject: `UniCertified | Ative Sua Conta ${response_findOne.get(
                  "name_user"
                )}`, // Subject line
